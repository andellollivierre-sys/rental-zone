...
export default function BookingForm() {
  ...
  useEffect(() => {
    const viewedBookingPage = sessionStorage.getItem('trz_viewed_booking_page');

    if (!viewedBookingPage) {
      sessionStorage.setItem('trz_viewed_booking_page', 'true');
  ...
  const trackBookingStarted = () => {
    try {
      ...
      sessionStorage.setItem('trz_booking_started', 'true');
      trackTrzEvent('booking_started');
    } catch (err) {
      console.warn('[TRZ TRACK] booking_started error:', err);
    }
  };
  ...
  const handlePackageSelect = (pkgKey) => {
    ...
    trackTrzEvent('package_selected', {
      package_type: packages[pkgKey].name
    });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    const { data: conflictingBookings, error: checkError } =
      await supabase
        .from('bookings')
        .select('*')
        .eq('event_date', formData.event_date)
    ...
    if (!checkError && conflictingBookings) {
      const hasConflict = conflictingBookings.some((b) => {
        return (
          formData.start_time < b.end_time &&
    ...
    try {
      storedTracking =
        JSON.parse(sessionStorage.getItem('trz_tracking')) || {};
    } catch (err) {
      storedTracking = {};
    }
    ...
    const utmMedium =
      ...
      (urlParams.get('fbclid') ? 'social_ad' : 'none');
    ...
    try {
      ...
      trackTrzEvent('booking_submitted', {
  ...
  const openWhatsApp = () => {
    ...
    trackTrzEvent('whatsapp_clicked', {
    ...
    return (
      ...
      >
        ...
        >
          ...
          >
            Your order is logged as Pending. Complete the
            step below via WhatsApp to secure your spot.
          </p>
          ...
          >
            ...
            >
              ...
              <br />
              🔒 <b>Required Deposit:</b> TT$
              {confirmedBooking.deposit}
              <br />
              💵 <b>Balance Due on Delivery:</b> TT$
              {confirmedBooking.balance}
            </div>
          </div>
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            Combination + Slide • 26x13x13ft • Up to 8 Kids
          </p>
        </div>
        ...
        >
          ...
          <div>
            ...
            >
              <div
                onClick={() =>
                  handlePackageSelect('2_hours')
                }
                style={{
                  border:
                    formData.package_type === '2_hours'
                      ? '2px solid #2563eb'
                      : '1px solid #cbd5e1',
                  background:
                    formData.package_type === '2_hours'
              ...
              <div
                onClick={() =>
                  handlePackageSelect('3_hours')
                }
                style={{
                  border:
                    formData.package_type === '3_hours'
                      ? '2px solid #2563eb'
                      : '1px solid #cbd5e1',
                  background:
                    formData.package_type === '3_hours'
              ...
              <div
                onClick={() =>
                  handlePackageSelect('full_day')
                }
                style={{
                  border:
                    formData.package_type === 'full_day'
                      ? '2px solid #2563eb'
                      : '1px solid #cbd5e1',
                  background:
                    formData.package_type === 'full_day'
            ...
            >
              {checkingAvailability ? (
                ...
                <div>
                  ...
                  >
                    ⚠️ Note for {formData.event_date}:
                  </span>
                ...
                >
                  ✅ Selected date is fully open! No
